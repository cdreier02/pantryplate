/* ----------------------------------------------------------------------------
   PantryPlate seed data — the single authored source of truth.

   This file is BOTH:
     1. The offline fallback bundled into the app (imported by PantryPlate.jsx),
        so a cold first launch with no network is never empty.
     2. The source for the deployed public/meals.json — run `npm run gen:meals`
        to (re)write public/meals.json from this array.

   To ADD meals to the live app you can either edit this file and re-run
   `npm run gen:meals`, or edit public/meals.json directly and push. The deployed
   meals.json is the live update channel; this array is the always-present baseline.
